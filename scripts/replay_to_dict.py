...
def to_dictUnit(unit):
    ...
    unit_dict = {
        key: unit.__getattribute__(key)
        for key in unit.__dir__()
...
def to_dictPlayerStatsEvent(event):
    ...
    return {
        key.replace("food", "supply"): event.__dict__[key]
...
def to_dictUnitPositionsEvent(event):
    return {
        "positions": {key.id: event.units[key] for key in event.units.keys()},
        "frame": event.frame,
        "second": event.second,
    }
...
def to_lean_dict(dict_replay, look_up=False, deduce=False):
    ...
    lean_replay["objects"] = {
        key: dict_to_values(object_keys, dict_replay["objects"][key])
        for key in dict_replay["objects"].keys()
    }

    lean_replay["player_stats"] = [
        dict_to_values(player_stat_keys, event) for event in dict_replay["player_stats"]
    ]
